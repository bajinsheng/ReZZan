...
for sub_dir in $(find ${TARGET_DIR} -mindepth 1 -maxdepth 1)
do
    cd ${DIR}/${sub_dir}
    pwd
    echo "target,Asan,ReZZan,ReZZan_lite" > ${DIR}/results/${CWE}_${sub_dir: -3}_good.csv
    rm -rf *.asan
    rm -rf *.rezzan
    rm -rf *.lite
    ...
    do
        ...
        echo "${binary}, ${asan_return_code}, ${rezzan_return_code}, ${lite_return_code}" >> ${DIR}/results/${CWE}_${sub_dir: -3}_good.csv
    done
